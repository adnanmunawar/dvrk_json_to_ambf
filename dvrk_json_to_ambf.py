...
class CreateAMBF:
    ...
    def get_path_and_file_name(self, filepath):
        ...
        if filepath.parts[0] == 'package:':
            ...
            abs_mesh_dir = os.path.join(os.path.dirname(CommonConfig.json_filepath), os.path.dirname(str(filepath))) + '/'
            filename = filepath.name
    ...
    def generate_ambf_config(self, json_robot):
        ...
        self._ambf_config['namespace'] = '/ambf/env/' + 'dvrk' + '/'
        ...
        json_dvrk_joints = OrderedDict()

        json_dvrk_joints = json_robot['DH']['links']

        num_dvrk_joints = len(json_dvrk_joints)

        ambf_links = OrderedDict()
        for n_link in range(0, num_dvrk_joints + 1):
            ambf_link = BodyTemplate()
        ...
        for n_joint in range(0, num_dvrk_joints):
            ...
            ambf_joint_data = ambf_joint.ambf_data
            joint_name = 'JOINT ' + str(n_joint)
            ambf_joint_data['name'] = joint_name
            ambf_joint_data['joint limits']['low'] = json_dvrk_joints[n_joint]['qmin']
            ambf_joint_data['joint limits']['high'] = json_dvrk_joints[n_joint]['qmax']
            a = json_dvrk_joints[n_joint]['A']
            alpha = json_dvrk_joints[n_joint]['alpha']
            theta = json_dvrk_joints[n_joint]['theta']
    ...
    def save_ambf_config(self, output_file):
        ...
        header_str = "# AMBF Version: %s\n" \
                     "# Generated By: dvrk_json_to_ambf \n" \
                     "# Link: %s\n" \
                     "# Generated on: %s\n" \
...
def main():
    setup_yaml()
    if len(sys.argv) > 1:
        CommonConfig.json_filepath= sys.argv[1]
        if os.path.exists(CommonConfig.json_filepath):
            print("Specified File: \"%s\"", CommonConfig.json_filepath)
        else:
            print("Filepath: \"%s\" does not exist on this machine, exiting", CommonConfig.json_filepath)
            exit()
    else:
        print("No JSON File Specified")
        exit()

    json_file = open(CommonConfig.json_filepath)
    robot_data = json.load(json_file)
    CommonConfig.robot_name = ''
    ...
    af_multi_body_config.generate_ambf_config(robot_data)

    # If two arguments already specified, save to the second argument
    save_to = '/Users/adnan/Desktop/dvrk_tool.yaml'
    if len(sys.argv) > 2:
        save_to = sys.argv[2]
